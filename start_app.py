...
def stop_streamlit():
    ...
    if os.name == "nt":  # Windows
        try:
            ...
            subprocess.run(["taskkill", "/F", "/IM", "python.exe", "/FI", "WINDOWTITLE eq streamlit*"], capture_output=True)
            subprocess.run(["taskkill", "/F", "/IM", "python.exe", "/FI", "COMMANDLINE eq *streamlit*"], capture_output=True)
